...
그러나 **보정 단계에서 이 구성이 검증에 적합하지 않음이 드러나 검증 스윕을 실행하지
않았다.** 2노드·원격 저장소 구성의 체크포인트 소요가 4.13 s로 측정되어 분석 임계값이
8.26 s가 되었는데, 같은 보정에서 얻은 δ로 계산한 예측 τ\*는 M = 300 s에서 4.7–5.5 s로
임계값의 0.6배 수준이었다. 즉 **예측된 최적점이 측정 가능한 주기 영역 아래에 놓여 실험적으로
식별할 수 없는 상태**였고, 정상 지연의 1/τ 회귀 또한 사용 가능한 점이 3개로 줄어 적합이
불안정하였다(R² = 0.10). 이 판단은 검증 데이터를 수집하기 전에 내려졌다.
