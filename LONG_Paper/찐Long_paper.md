...
δ·ℓ₀·D·μ_d를 측정해 **동결**한 뒤 τ\*를 계산하고, 그 주변에 0.6τ\*·τ\*·1.8τ\*를 포함하는
8점 격자를 배치하여 각 3회 반복하며, 검증 런의 데이터로는 어떤 파라미터도 재추정하지 않고,
판정 기준(|τ\*_실측 − τ\*_예측|/τ\*_예측 ≤ 0.25 및 양쪽 가지의 증가)을 데이터 수집 전에
확정하는 절차였다. 모든 런에 장애를 정확히 4회 주입해 장애 노출률을 ν = 1/300 s⁻¹로
맞추되 장애 시각은 연속 균등분포에서 추출하여, 유효 장애 간격을 고정하면서도 체크포인트
대비 위상은 무작위로 유지하도록 설계하였다.

그러나 **보정 10런을 수행한 결과 이 구성에서는 검증에 필요한 파라미터를 얻을 수 없음이
드러나 검증 스윕을 실행하지 않았다.** 2노드·원격 저장소 구성의 체크포인트 소요가 4.13 s로
측정되어 분석 임계값이 8.26 s가 되었고, 이 때문에 무장애 런 6개 중 2개(τ = 3.9, 5.7 s)가
포화 구간으로 빠져 1/τ 회귀에 남은 점이 4개, 적합도가 R² = 0.04에 그쳤다. 장애 런에서도
계획한 12개 에피소드 중 중복·절단을 제외하고 2개만 남았으며, 그 D_eff 중앙값은 100 s
(범위 7.9–192 s)로 이는 복구 공백 시간이 아니라 **다음 장애 전에 회복하지 못한 구간**을
반영한 값이다.

즉 이 구성에서는 δ와 D를 신뢰할 수 있게 추정할 수 없었고, 체크포인트 소요가 정하는 분석
임계값(8.26 s)이 어떤 합리적 파라미터 조합으로 계산하더라도 예측 최적점보다 크게 나타났다.
**예측된 최적점이 측정 가능한 주기 영역 아래에 놓여 실험적으로 식별할 수 없는 상태**였던
것이다. 이 판단은 검증 데이터를 수집하기 전에 내려졌다.

보정 설계 자체의 한계도 함께 기록해 둔다. 보정의 주기 격자(4–30 s)와 장애 간격(600 s에
3회)은 체크포인트 소요를 2.4 s로 예상하고 정한 것인데 실측이 4.13 s였고, 그 결과 격자의
일부가 임계값 아래로 떨어지고 에피소드가 서로 겹쳤다. 재시도 시에는 체크포인트 소요를 먼저
측정한 뒤 격자와 장애 간격을 정해야 한다.

따라서 본 논문은 τ\*의 독립 검증을 **미완료 한계로 명시**한다. 이를 수행하려면 예측 최적점이
경험적 사용 가능 영역 위에 놓이는 구성 — 체크포인트 소요가 짧거나 δ가 상대적으로 큰 조건,
그리고 더 긴 평균 장애 간격 — 을 먼저 확보해야 한다.
