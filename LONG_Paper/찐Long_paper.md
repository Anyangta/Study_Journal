...
| 조건 | δ (s) | D (s) | M (s) | 예측 τ\* (s) | 주기 바닥 2d (s) | 실측 τ 범위 (점수) |
|---|---|---|---|---|---|---|
...
소요가 8.5 s여서 분석 바닥(2d=17.1 s)이 예측값 8.3 s보다 크고, 200 MB·빠름 조건은 실측
최소 주기가 16 s로 예측값 7.3 s보다 길다. 즉 **이 구성들에서는 지연 최적 주기가 체크포인트
소요가 정하는 주기 바닥보다 짧다.** 이는 IV.C-1의 관찰과 같은 방향의 결과로, 상태가 크거나
저장소가 느릴수록 지연 관점의 최적 주기를 실제로 채택할 수 없게 됨을 뜻한다. 반대로 말하면
지연 최적 주기를 실현하려면 체크포인트 소요 자체를 줄이는 것이 선결 조건이다.
...
분석 바닥보다 짧아 측정 범위 밖에 있었다. 이를 확인하려면 체크포인트 소요가 충분히 짧은
...
- **τ\* 검증 시도 결과(2026-09-11)**: `summary.csv` 255런에서 조건별로 예측 τ\*와 실측
  ...
  범위 안에 드는 조건은 100 MB·빠름 하나(4런)였다. 나머지는 예측값이 주기 바닥(2d)보다
