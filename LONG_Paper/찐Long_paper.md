...
지수 약 0.41–0.34) 장애가 드물수록 두 방식의 차이가 커진다. 제안한 backlog drain 기하
...
**Figure 2.** 대표 복구 에피소드(200k key, ρ=0.30, τ=89 s)의 지연·처리량 시계열. 장애 후
backlog가 제거되면서 레코드 지연이 감소하는 삼각형 형태의 recovery trajectory를 보여준다.
아래 패널이 메커니즘을 직접 보여준다 — catch-up 동안 처리량이 도착률의 약 3배로 올라갔다가
backlog가 다 빠지면 λ로 돌아온다. 즉 backlog는 `μ_d − λ`로 줄어든다. 여러 에피소드에 걸친
면적의 제곱 관계는 Figure 3에서 검증한다.
...
정렬되며, 그 관계는 약 세 자릿수(0.8–712 M record·s) 범위에 걸쳐 유지된다. 즉 기하식은
...
여러 에피소드에 걸쳐 quadratic backlog drain 관계가 약 세 자릿수 범위에서 유지된다.
...
약 0.41, 1시간에서 약 0.36, M=1일에서 약 0.34이며 1/3로 점근한다(기준 파라미터
δ=1.907 s, D=4.28 s, ρ=0.5, ρ_d=0.513, δ_cap=0.22 s). 그 결과 두 주기의 비는
M=30초에서 1.3배, 1시간에서 2.3배, 1일에서 3.7배로 벌어진다. 따라서 두 예측 주기의 격차는 장애가 드물수록 벌어진다.
...
지연 최적 주기는 √M보다 느리게(국소 0.41–0.34) 증가한다. Flink/Kafka 실험에서 개별 장애
...
- **ρ_d와 국소 지수 정정 (2026-09-13)**: `make_fig4_scaling.py`의 `RHO_D`가
  **0.5563 — Table IV의 τ*와 맞도록 역산한 값**이었음. 기준 arm(200k·ρ0.50·빠른
  저장소 밴드)의 장애 run 6개에서 측정한 **0.5134**(λ=54811, μ_d=106761 중앙값)로
  교체하고 Figure 4 재생성. 또 논문이 인용하던 국소 지수 **0.43/0.35는 논문 자신의
  기준 파라미터에서 재현되지 않음** — ρ_d를 뭘 쓰든 0.41/0.36/0.34가 나옴
  (역산값 0.5563으로도 0.411). 지수와 비율(1.4/2.4/3.8 → **1.3/2.3/3.7**)을 전부
  정정. 재계산값이 격차를 더 작게 보므로 **보수적 방향**임. δ_cap=0.22 s(본문
  0.15–0.22의 상한)도 Figure 4 캡션에 명시.
- **Figure 2 재생성 (2026-09-13)**: 기존 그림이 `m10_p1024_k200k_s32000_r50_c32_m55_0`
  에서 그려져 있었는데, 그 run은 `steady_lat=102 s`·`D_eff=98 s`로 **복구에 실패한
  run**이었다. 그림에 지연이 33 s에서 정체하다 40 s에 재상승하는 궤적이 찍혀 있어
  "삼각형으로 감소한다"는 캡션과 정반대였다. 원인은 `plot.py`가 품질 조건 없이
  "장애 3회 이상 + τ∈{16,32}s"에 맞는 **첫 run**을 집는데, 문자열 정렬에서
  `m10`이 `m2`보다 앞서 걸린 것. `pick_trace_run()`을 추가해 (과부하·불안정·포화
  제외, `μ_d>λ`, `D_eff≤12 s`, 배수가 창 안에서 완료) 조건으로 고르게 하고
  `m9_p1024_k200k_s32000_r30_c80_m55_0`의 4번 에피소드로 재생성.
  새 그림은 89 s → 0으로 삼각형 감소하고, 처리량이 catch-up 중 λ의 3배로 올랐다가
  λ로 복귀하는 것까지 보인다. 캡션에 그 사실을 추가.
- **Figure 3 자릿수 정정 (2026-09-13)**: "네 자릿수(10⁰–10⁴ M record·s)"는 틀림.
  `검증실험/fig3_episodes.csv`의 `in_figure3=1` 218개는 **0.83–712 M record·s로
  약 2.9 자릿수**다. 전체 420개(제외분 포함)면 4.5 자릿수지만 그림에 찍히는 것은
  218개다. 영문·한글 모두 "약 세 자릿수(0.8–712 M record·s)"로 수정.
