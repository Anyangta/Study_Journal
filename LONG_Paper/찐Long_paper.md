...
| 검증 캠페인 | 2노드 보정 10 run + held-out 검증 24 run (IV.C) |
...
보관하였으며, 특히 (iii)에 해당하는 run은 IV.D-4의 안정성 논의에 사용한다.

### C. Held-out 검증 프로토콜

III.C에서 유도한 최적 주기 τ\*는 모델의 출력이므로, 그 위치가 실제 최소 지연 주기와
일치하는지는 **모델을 적합한 데이터와 분리된 데이터**로 확인해야 한다. 이를 위해
2노드·원격 저장소 구성에서 별도의 검증 실험을 다음 절차로 수행하였다. 절차와 판정 기준은
**검증 데이터를 수집하기 전에 확정하여 기록**하였다.

**1) 보정–검증 분리.** 먼저 보정(calibration) 런에서 이 구성의 δ, ℓ₀, D, μ_d를 측정한다.
무장애 런으로 정상 지연의 1/τ 회귀에서 δ와 ℓ₀를, 장애 런으로 에피소드별 D와 μ_d를 얻는다.
이 시점에 **모든 모델 파라미터를 동결**하고 τ\*를 계산한다. **검증 런의 데이터로는 어떤
파라미터도 재추정하지 않는다.**

**2) 검증 런 사양.** 런당 측정 1200초, 워밍업 90초, 장애 **정확히 4회**를 주입한다. 따라서
모든 런의 장애 노출률은 ν = 4/1200 = **1/300 s⁻¹**로 같다(등가 장애 간격 M = 300초).
이는 "300초마다 장애를 준다"는 뜻이 아니라 런마다 장애 횟수를 고정해 노출률을 맞춘다는
뜻이다. 장애 시각은 [120, 1060]초 구간의 연속 균등분포에서 최소 간격 180초 조건으로
추출하므로, 체크포인트 주기(수~수십 초)와 의도적인 위상 고정이 발생하지 않는다.

장애 횟수를 고정한 이유는 선행 분석의 실패 경험 때문이다. 장애를 단순 renewal로 주입하면
런마다 실제 장애 횟수가 달라져 유효 장애 간격이 흩어지고, 이를 공통 M으로 정규화하려면
장애 비용 ∝ 1/M 가정을 먼저 써야 하는데 그 가정이 곧 검증 대상이므로 순환이 된다.

**3) 주기 격자.** 동결된 τ\*를 기준으로 8개 지점을 배치하되, **0.6τ\*, τ\* 부근, 1.8τ\***
를 반드시 포함하고 나머지는 0.45τ\*–2.5τ\* 범위를 로그 간격으로 채운다. 각 주기를 3회
반복하고 실행 순서를 무작위화하여, 저장소 성능 변화가 주기와 상관되지 않도록 한다.

**4) 사전 정의한 판정 기준.** 각 주기에서 3런의 record-weighted mean latency의 **중앙값**을
그 주기의 대표값으로 하고, **사전에 정한 8개 격자점 중 대표값이 최소인 점**을 실측 최적
주기로 정의한다. 곡선을 새로 적합하여 최소점을 구하지 않는다(검증에 모델을 하나 더 도입하게
되므로). 판정은 다음과 같다.

- **주 기준**: `|τ*_실측 − τ*_예측| / τ*_예측 ≤ 0.25`
- **보조 기준**: `L(τ*) < L(0.6τ*)` 그리고 `L(τ*) < L(1.8τ*)` — 양쪽에서 증가하는 U자 확인
- **보조**: 동결 파라미터로 계산한 L̄(τ) 곡선과 실측 곡선의 비교. 곡선의 절대 수준은
  ℓ₀·δ·D·μ_d가 모두 정확해야 하므로 주 기준으로 삼지 않는다.

M = 300초는 예측 최적점을 IV.B의 경험적 분석 임계값(τ < 2d) 위로 이동시키기 위해 선택하였다.
τ\*는 M에 대해 증가하지만 이 임계값은 체크포인트 소요로 정해지므로, M을 키우면 최적점이
측정 가능한 영역으로 올라온다. 이 선택은 결과를 본 뒤가 아니라 설계 단계에서 정하였다.

**5) 사후 확인.** 검증 후 각 장애의 체크포인트 경과 비율 `a/τ` 분포가 특정 위상에 몰리지
않는지 확인하여 `a ~ U(0,τ)` 가정을 점검하고, 실행 순서와 체크포인트 소요의 상관으로 저장소
성능 변화 여부를 확인한다.
...
**최적 주기 τ\*의 위치는 검증되지 않았다.** Table IV에서 보듯 100 MB·빠름 조건에서만
측정된 최소점(4.2 s)이 예측값(4.6 s)과 한 격자 이내로 일치했고, 그마저 스윕의 왼쪽 끝이라
최적점의 한쪽 가지만 관측되었다. 나머지 조건에서는 예측 τ\*가 체크포인트 소요가 정하는
분석 임계값보다 짧아 측정 범위 밖에 있었다. 이를 확인하려면 체크포인트 소요가 충분히 짧은
구성(작은 상태 또는 빠른 저장소)에서 예측 τ\* 주위를 촘촘히 스윕하고, 조건 내 평균 장애
간격을 고정한 추가 실험이 필요하다.
...
요인의 인과로 귀속하지 않는다. 향후 예측된 최적 주기 자체를 검증하기 위해 체크포인트 소요가 짧은 구성에서
예측값 주위를 촘촘히 스윕하는 실험, 로컬/원격 저장소를 독립 변경하는 실험, 더 많은 노드,
실제 workload, 다양한 장애 유형으로 일반성을 검증할 계획이다. 본 연구의 원자료(255회 실험, 13,000회 이상 체크
...
- **6쪽 압축**: 2단에 넣어 넘치면 IV.D-4(저장소/안정성)를 2–3문장으로 줄이고, Table III·IV 중
...
- **τ\* 검증 시도 결과(2026-09-11)**: `summary.csv` 255런에서 조건별로 예측 τ\*와 실측
  ...
  짧았다. 이 결과를 **Table IV와 IV.E 한계**로 정직하게 서술했다. 조건별 M_eff가 48~900초로
