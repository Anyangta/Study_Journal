...
스냅샷·순간 지연으로 많은 레코드의 지연을 늘리기 때문이다. Table III는 네 조건의 체크포인트
...
1시간에서 2.4배, 1일에서 3.8배로 벌어진다. 따라서 장애가 드문 환경일수록 용량/낭비 기준으로
택한 긴 주기가 지연 민감형 응용에는 과도하게 길어진다.
...
분석 임계값이 8.26 s가 되었고, 이는 보정이 예측한 최적점보다 컸다. 보정 데이터로는 δ와 D를
신뢰할 수 있게 추정할 수 없었다(1/τ 회귀에 남은 점 4개, R² = 0.04; 사용 가능한 장애
에피소드 2개). **이 판단은 검증 데이터를 수집하기 전에 내려졌으며**, τ\*의 독립 검증은
...
검증할 계획이다. 본 연구의 원자료(255회 실험, 13,000회 이상 체크
포인트)와 분석 스크립트는 재현을 위해 보존하였다.
