...
| 보정 캠페인 | 2노드 보정 10 run (IV.D의 사전 등록 검증 시도) |
...
이를 메우기 위해 별도의 held-out 검증을 **사전 등록 방식**으로 계획하였다. 보정 런에서
...
요인의 인과로 귀속하지 않는다. 예측된 최적 주기 자체의 독립 검증은 사전 등록 절차까지 마련하였으나, 보정 단계에서 해당
...
- **held-out 검증 최종 결정(2026-09-11 19시)**: 사전 등록 프로토콜(보정→동결→격자→검증)을
  세우고 2노드 보정 10런을 실행했으나, 보정에서 체크포인트 소요 4.13 s(임계 8.26 s)·
  예측 τ\* 4.7–5.5 s(비율 0.6)가 확인되어 **검증 스윕을 실행하지 않고 중단**.
  δ 적합도 사용 가능 점이 3개로 줄어 R²=0.10으로 불안정. **검증 데이터 수집 전에 내린
